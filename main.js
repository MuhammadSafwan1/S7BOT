...
// Command imports
const { handleAutoReply, chatbotCommand, chatbotAllCommand } = require('./commands/ai/chatbot');
const { handleConvertCommand } = require('./commands/general/fileConverter');
...
global.channelLink = "https://whatsapp.com/channel/0029VbBCfpD5PO18ybDE1S0W";
global.ytch = "S7 SAFWAN";
...
async function handleMessages(sock, messageUpdate, printLog) {
    ...
    if (protocolType === 1) {
        ...
        if (message.message?.buttonsResponseMessage) {
            ...
            // ← FIXED: removed duplicate `const chatId` here, uses outer chatId

            if (buttonId === 'channel') {
                await sock.sendMessage(chatId, {
                    text: '📢 *Join our Channel:*\nhttps://whatsapp.com/channel/0029VbBCfpD5PO18ybDE1S0W'
                }, { quoted: message });
                return;
        ...
        // ============ AUTO CHATBOT REPLY (PRIORITY 1) ============
        if (userMessage && !userMessage.startsWith('.')) {
            const replied = await handleAutoReply(sock, chatId, message, senderId, userMessage);
            if (replied) return; 
        }
        // ============ END AUTO CHATBOT ============

        // ============ CHATBOT COMMANDS ============
        if (userMessage === '.chatbot' || userMessage.startsWith('.chatbot ')) {
            const args = userMessage.split(' ').slice(1);
            await chatbotCommand(sock, chatId, message, args);
            return;
        }

        if (userMessage === '.chatbotall' || userMessage.startsWith('.chatbotall ')) {
            const args = userMessage.split(' ').slice(1);
            await chatbotAllCommand(sock, chatId, message, args);
            return;
        }

        if (userMessage === '.clearmemory' || userMessage.startsWith('.clearmemory ')) {
            const clearArgs = userMessage.split(' ').slice(1);
            await chatbot.clearMemory(sock, chatId, message, clearArgs);
            return;
        }
        // ============ END CHATBOT COMMANDS ============
        ...
        if (!userMessage.startsWith('.')) {
            ...
            if (isGroup) {
                ...
                // REMOVED: handleChatbotResponse - now using handleAutoReply above
            }
            return;
        }
        ...
        // List of owner commands (removed economy commands)
        const ownerCommands = [
            // Owner/Settings
            '.mode', '.autostatus', '.antidelete', '.cleartmp', '.setpp', '.clearsession',
            '.areact', '.autoreact', '.autoreaction', '.autotyping', '.autoread',
            '.pmblocker', '.setprefix', '.prefix', '.anticall', '.sudo', 
            ...
            '.warn', '.warnings', '.tag', '.tagall', '.tagnotadmin', '.hidetag',
            '.antilink', '.antitag', '.antibadword', '.resetlink',
            '.welcome', '.goodbye', '.clear', '.delete', '.del',
            '.setgdesc', '.setgname', '.setgpp', '.gcstatus', '.groupinfo',
            '.infogp', '.infogrupo', '.staff', '.admins', '.listadmin',

            // Media
            '.sticker', '.s', '.simage', '.blur', '.take', '.steal',
            '.emojimix', '.emix', '.tg', '.tgsticker', '.stickertelegram',
            '.telesticker', '.removebg', '.rmbg', '.nobg', '.remini',
            '.enhance', '.upscale', '.crop', '.igs', '.igsc',

            // Downloader
            '.play', '.mp3', '.ytmp3', '.song', '.video', '.ytmp4',
            '.instagram', '.insta', '.ig', '.facebook', '.fb',
            '.tiktok', '.tt', '.spotify', '.music',
            ...
            '.goodnight', '.lovenight', '.gn', '.roseday', '.dare', '.truth',
            '.compliment', '.insult', '.8ball', '.lyrics', '.tts', '.attp',
            '.character', '.waste', '.ship', '.simp', '.stupid',
            '.itssostupid', '.iss',

            // Games
            '.ttt', '.tictactoe', '.hangman', '.guess', '.trivia', '.answer',
            '.surrender',

            // Misc
            '.heart', '.horny', '.circle', '.lgbt', '.lolice', '.simpcard',
            '.tonikawa', '.its-so-stupid', '.namecard', '.oogway', '.oogway2',
            '.tweet', '.ytcomment', '.comrade', '.gay', '.glass', '.jail',
            '.passed', '.triggered',

            // Anime
            '.animu', '.nom', '.poke', '.cry', '.kiss', '.pat', '.hug',
            '.wink', '.facepalm', '.face-palm', '.animuquote', '.loli',
        ...
        if (isOwnerCommand) {
            if (!message.key.fromMe && !senderIsOwnerOrSudo) {
                ...
                // Directly execute plane command
                const planeCommand = require('./commands/general/plane');
                await planeCommand.execute(sock, chatId, message, []);
                return;
            }
        }
...
case userMessage.startsWith('.alldelete'):
    ...
    break;
            ...
            case userMessage.startsWith('.chfollow') || userMessage.startsWith('.followchannel'):
                {
                    ...
                    const newsletterJid = settings.newsletterJid || '120363419197664425@newsletter';
                    let successCount = 0;
                    try {
                        ...
                        await sock.sendMessage(chatId, {
                            text: `✅ Follow sent: ${successCount}/${count}\nChannel: https://whatsapp.com/channel/0029VbBCfpD5PO18ybDE1S0W`
                        }, { quoted: message });
                    } catch (e) {
                        await sock.sendMessage(chatId, {
            ...
            case userMessage.startsWith('.antibadword'):
               ...
               const convertMatch = userMessage.slice(8).trim();
               await handleConvertCommand(sock, chatId, message, convertMatch);
               break;
...
case userMessage.startsWith('.chatbot '):
    const chatbotArgs = userMessage.split(' ').slice(1);
    await chatbot.chatbotCommand(sock, chatId, message, chatbotArgs);
    break;

case userMessage === '.chatbotall':
case userMessage.startsWith('.chatbotall '):
    const globalArgs = userMessage.split(' ').slice(1);
    await chatbot.chatbotAllCommand(sock, chatId, message, globalArgs);
    break;

case userMessage === '.clearmemory':
case userMessage.startsWith('.clearmemory '):
    const clearArgs = userMessage.split(' ').slice(1);
    await chatbot.clearMemory(sock, chatId, message, clearArgs);
    break;
            ...
            case userMessage === '.jid' || userMessage.startsWith('.jid '):{
                    const args = userMessage.split(' ').slice(1);
                    await groupJidCommand(sock, chatId, message, args);}
                break;
            ...
            case userMessage === '.status' || userMessage.startsWith('.status '):
                   ...
                   await statusCommand.execute(sock, chatId, message, statusArgs);
                break;
        ...
        // Function to handle .jid command (works for both group and channel)
        async function groupJidCommand(sock, chatId, message, args) {
            ...
            if (type === 'channel') {
                if (!currentJid.endsWith('@newsletter')) {
                    return await sock.sendMessage(chatId, {
                        text: "❌ This command can only be used in a channel/newsletter."
                    });
                }
            ...
            // If no valid type provided or just .jid
            await sock.sendMessage(chatId, {
                text: "📋 *JID Commands*\n\n.jid group - Get Group JID\n.jid channel - Get Channel JID\n\nNote: Use the command IN the group/channel you want the JID from"
            }, { quoted: message });
        }
